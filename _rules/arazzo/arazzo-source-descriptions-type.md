...
ruleyaml: >
  arazzo-source-descriptions-type:
    ...
    prompt: "You are editing an Arazzo document to satisfy the Spotlight API
      governance rule 'arazzo-source-descriptions-type' (Arazzo Source
      Descriptions Type). Requirement: Source Description \"type\" should be
      present. To fix: Ensure `type` is present and non-empty at each matching
      location. This rule is evaluated at the JSONPath `$.sourceDescriptions[*]` —
