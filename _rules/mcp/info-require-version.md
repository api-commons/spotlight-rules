...
topic: []
owasp: []
reference: https://spotlight-rules.com/spec/rules/mcp/info-require-version/
prompt: "You are editing a MCP document to satisfy the Spotlight API governance
  rule 'info-require-version' (Info Require Version). Requirement: The `version`
  property should be present. To fix: Ensure `version` is present and non-empty
