...
ruleyaml: >
  capability-mcp-capabilities-not-empty:
    ...
    prompt: "You are editing a MCP document to satisfy the Spotlight API governance
      rule 'capability-mcp-capabilities-not-empty' (Capability MCP Capabilities
      Not Empty). Requirement: capabilities should declare at least one of tools,
      resources, or prompts. To fix: Adjust the targeted value so it conforms to
      ...
      keep the document valid MCP. Return only the complete corrected document,
      with no commentary."
---
