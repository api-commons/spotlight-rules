...
ruleyaml: >
  operation-id-camel-case:
    ...
    description: Operation identifiers provide a unique way to identify each
      ...
      friction when generating SDKs and automating around APIs.
    message: Operation Identifier MUST Be camelCase
    severity: info
    given: $.paths.*[get,post,patch,put,delete].operationId
    then:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      ...
      Requirement: Operation identifiers provide a unique way to identify each
      individual API, and requiring them to have consistent casing reduces
      friction when generating SDKs and automating around APIs. To fix: Ensure the
      targeted value matches the regular expression `^[a-z]+(?:[A-Z][a-z]+)*$`;
      rewrite any value that does not. This rule is evaluated at the JSONPath
      `$.paths.*[get,post,patch,put,delete].operationId` — inspect every location
