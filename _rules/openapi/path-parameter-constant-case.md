...
ruleyaml: >
  path-parameter-constant-case:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'path-parameter-constant-case' (Path Parameter Constant
      Case). Requirement: Path parameters MUST follow CONSTANT_CASE (screaming
      ...
      This rule is evaluated at the JSONPath `$..parameters[?(@.in ==
      'path')].name` — inspect every location it matches and correct only what
