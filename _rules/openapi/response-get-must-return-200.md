...
ruleyaml: >
  response-get-must-return-200:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'response-get-must-return-200' (Response Get Must Return
      200). Requirement: GET operations must always return 200 status code. To
      fix: Ensure `200` is present and non-empty at each matching location. This
      rule is evaluated at the JSONPath `$.paths[*].get.responses` — inspect every
