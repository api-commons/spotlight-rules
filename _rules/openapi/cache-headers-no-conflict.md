...
slug: cache-headers-no-conflict
title: Cache Headers No Conflict
severity: info
...
description: 'Cache usage SHOULD be extensively detailed in the `description`
  ...
  * the presence of following keywords in the `description`: `max-age`,
  ...
  euristic cache like described in RFC7234. It requires exactly one of
  ["Expires","Cache-Control"] to be present (evaluated at
  `$.[responses][?(@property[0] == "2" )][headers]`). Severity: info.'
experience:
  - reliability
  - consistency
spec:
