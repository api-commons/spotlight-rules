...
prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
  governance rule 'response-get-define-500' (Response Get Define 500).
  Requirement: GET responses should have a 500 internal server erorr HTTP status
  ...
  the JSONPath `$.paths.*.get.responses` — inspect every location it matches and
...
ruleyaml: >
  response-get-define-500:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'response-get-define-500' (Response Get Define 500).
      Requirement: GET responses should have a 500 internal server erorr HTTP
