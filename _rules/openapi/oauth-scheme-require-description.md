...
slug: oauth-scheme-require-description
title: Oauth Scheme Require Description
severity: warn
...
description: 'Json Web Tokens RFC7519 is a compact, URL-safe means of
  ...
  conformato to the specifications included in RFC8725. ```. It requires the
  `description` field to be present and non-empty, and requires the
  `description` field to match the pattern `.*RFC8725.*` (evaluated at
  `$.[securitySchemes][?(@.type=="oauth2")]`). Severity: warn.'
experience:
  - security
  - documentation
spec:
