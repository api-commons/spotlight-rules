...
ruleyaml: >
  response-body-must-be-object:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'response-body-must-be-object' (Response Body Must Be
      Object). Requirement: Response bodies must be an object type. To fix: Ensure
      the targeted value matches the regular expression `object`; rewrite any
