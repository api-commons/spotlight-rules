...
ruleyaml: >
  path-no-technology-leak-in:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'path-no-technology-leak-in' (Path No Technology Leak In).
      Requirement: A resource MUST NOT leak or expose format or
      technology-specific information at any point in the path. To fix: Ensure the
      targeted value does NOT match the regular expression
      `(.php|.asp|.jsp|.cgi|.psp|.json|.xml)`; rename or rewrite any value that
