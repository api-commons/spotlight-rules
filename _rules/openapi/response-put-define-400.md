...
ruleyaml: >
  response-put-define-400:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'response-put-define-400' (Response Put Define 400).
      Requirement: PUT responses should have a 400 not found HTTP status code,
      communicating nothing was found to consumers. To fix: Ensure `400` is
      present and non-empty at each matching location. This rule is evaluated at
      the JSONPath `$.paths.*.put.responses` — inspect every location it matches
