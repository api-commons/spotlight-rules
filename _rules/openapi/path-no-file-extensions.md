...
ruleyaml: >
  path-no-file-extensions:
    ...
    reference: https://spotlight-rules.com/spec/rules/openapi/path-no-file-extensions/
    description: API paths should not include file extensions like .json or .xml.
      Content negotiation should be handled through Accept headers rather than
      path suffixes.
    message: Paths MUST NOT Contain File Extensions
    severity: info
    given: $.paths
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'path-no-file-extensions' (Path No File Extensions).
      Requirement: API paths should not include file extensions like .json or
      .xml. Content negotiation should be handled through Accept headers rather
      than path suffixes. To fix: Ensure the targeted value does NOT match the
      regular expression `\\.(json|xml|html|yaml|yml|csv|pdf)$`; rename or rewrite
      any value that does. This rule is evaluated at the JSONPath `$.paths` —
