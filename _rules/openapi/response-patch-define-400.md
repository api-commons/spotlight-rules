...
ruleyaml: >
  response-patch-define-400:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'response-patch-define-400' (Response Patch Define 400).
      Requirement: PATCH operations should define a 400 Bad Request response for
      validation errors on partial update payloads. To fix: Ensure `400` is
      present and non-empty at each matching location. This rule is evaluated at
      the JSONPath `$.paths[*].patch.responses` — inspect every location it
