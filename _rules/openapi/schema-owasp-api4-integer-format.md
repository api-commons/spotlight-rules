...
ruleyaml: >
  schema-owasp-api4-integer-format:
    ...
    tags:
      ...
      - owasp:api4
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'schema-owasp-api4-integer-format' (Schema OWASP API4
      Integer Format). Requirement: Integer properties should declare a format
      (int32 / int64) so their range is bounded (OWASP API4 — unrestricted
      resource consumption). To fix: Ensure `format` is present and non-empty at
      each matching location. This rule is evaluated at the JSONPath
      `$..properties[?(@ && @.type == 'integer')]` — inspect every location it
