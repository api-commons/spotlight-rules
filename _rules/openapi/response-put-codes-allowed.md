...
ruleyaml: >
  response-put-codes-allowed:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'response-put-codes-allowed' (Response Put Codes Allowed).
      Requirement: PUT operations should not return 200 or 201 status codes. To
      fix: Ensure `@key` does NOT match the regular expression `^(200|201)$`;
