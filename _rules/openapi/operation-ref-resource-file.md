...
ruleyaml: >
  operation-ref-resource-file:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'operation-ref-resource-file' (Operation Ref Resource File).
      Requirement: Endpoint must a $ref to a file in resources/. To fix: Ensure
      the targeted value matches the regular expression `^resources/.*yml$`;
      rewrite any value that does not. This rule is evaluated at the JSONPath
      `$.paths.*.$ref` — inspect every location it matches and correct only what
