...
ruleyaml: >
  tag-no-camel-case:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'tag-no-camel-case' (Tag No Camel Case). Requirement:
      Detects a tag with camelCase (personalItem) text within it. To fix: Ensure
      the targeted value does NOT match the regular expression
      `/^[a-z]+[A-Z][a-z]+$/g`; rename or rewrite any value that does. This rule
