...
ruleyaml: >
  response-get-must-return-body:
    ...
    prompt: "You are editing an OpenAPI document to satisfy the Spotlight API
      governance rule 'response-get-must-return-body' (Response Get Must Return
      Body). Requirement: A `GET` operation must return a response body. To fix:
      Ensure `content` is present and non-empty at each matching location. This
      rule is evaluated at the JSONPath `$.paths[*].get.responses[*]` — inspect
