...
prompt: "You are editing an AsyncAPI document to satisfy the Spotlight API
  governance rule 'asyncapi-3-channel-no-query-nor-fragment' (Asyncapi 3 Channel
  No Query Nor Fragment). Requirement: Channel address must not include query
  (\"?\") or fragment (\"#\") delimiter. To fix: Ensure `address` does NOT match
