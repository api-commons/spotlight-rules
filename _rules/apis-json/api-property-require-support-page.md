...
builtin: false
ruleyaml: >
  api-property-require-support-page:
    title: API Property Require Support Page
    reference: https://spotlight-rules.com/spec/rules/apis-json/api-property-require-support-page/
    description: This property ensures that there is a support page available for an
      API, providing direct and in-direct support opportunities for each API or
      for entire API platform.
    message: Has Support Page
    severity: info
    given:
      - $.apis.*.properties.*
    ...
    prompt: "You are editing an APIs.json document to satisfy the Spotlight API
      governance rule 'api-property-require-support-page' (API Property Require
      Support Page). Requirement: This property ensures that there is a support
      page available for an API, providing direct and in-direct support
      opportunities for each API or for entire API platform. To fix: Ensure `type`
      does NOT match the regular expression `\\b(support|Support)\\b`; rename or
