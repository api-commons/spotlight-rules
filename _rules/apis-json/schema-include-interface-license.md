...
ruleyaml: >
  schema-include-interface-license:
    ...
    prompt: "You are editing an APIs.json document to satisfy the Spotlight API
      governance rule 'schema-include-interface-license' (Schema Include Interface
      License). Requirement: This property ensures that an API Commons interface
      license exists for an API, providing a machine-readable reference for an
      API, as well as data, backend, and front-end code. To fix: Ensure `type`
      does NOT match the regular expression
      `\\b(api-license|License|license|InterfaceLicense)\\b`; rename or rewrite
