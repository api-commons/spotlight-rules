...
ruleyaml: >
  schema-include-github-action:
    ...
    prompt: "You are editing an APIs.json document to satisfy the Spotlight API
      governance rule 'schema-include-github-action' (Schema Include Github
      Action). Requirement: This property ensures that a GitHub Actions CI/CD
      pipeline is available for an API, providing a link to the pipeline YAML
      artifact, which can be used to automate and govern the API as part of the
      build process. To fix: Ensure `type` does NOT match the regular expression
      `\\b(github-actions|GitHubActions)\\b`; rename or rewrite any value that
